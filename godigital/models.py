...
class OperatorAirline(models.Model):
    airline_name = models.CharField(max_length=255)
    # Add any other fields relevant to the customer
...
class Aircrafts(models.Model):
    ...
    operator_airline = models.ForeignKey(OperatorAirline, on_delete=models.CASCADE, related_name='aircrafts', null=True, blank=True)

    def __str__(self):
        return f"{self.aircraft_reg} - {self.get_fleet_display()}-{self.operator_airline}"
...
class ServiceLog(models.Model):
    ...
    operator_airline = models.ForeignKey(OperatorAirline, on_delete=models.CASCADE, null=True, blank=True)

    def __str__(self):
        return f"{self.date} - {self.flt_no}"